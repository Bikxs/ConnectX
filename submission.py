...
class ConnectX(py_environment.PyEnvironment):
    def __init__(self, switch_prob=0.5, agent2="negamax"):
        ...
        self._observation_spec = {
            'board': array_spec.BoundedArraySpec(shape=(self.rows, self.columns), dtype=np.int32, minimum=0,
                                                 maximum=2, name='board'),
            'mark': array_spec.BoundedArraySpec(shape=(), dtype=np.int32, minimum=1, maximum=2, name='mark')}
    ...
    def extract_obs(self, obs):
        return {'board': np.array(obs['board'], dtype=np.int32).reshape((self.rows, self.columns)),
                'mark': np.array(obs['mark'], dtype=np.int32)}
...
def train(policy_name, vs_agent):
    print(f'Print tensorflow version:{tf.version.VERSION}')
    print(f'\nTraining {policy_name} vs {vs_agent}')
    ...
    preprocessing_layers = {
        ...
        # tf.keras.models.Sequential([tf.keras.layers.Conv2D(16, 4), tf.keras.layers.Flatten()]),
        'mark': tf.keras.layers.Flatten()
    }
    preprocessing_combiner = tf.keras.layers.Concatenate(axis=-1)

    fc_layer_params = [1024, 1024, 1024, 1024]
    dropout_layer_params = [.1, .1, .1, .1]
    ...
    policy_dir = os.path.join(POLICY_DIR, f'{policy_name}_{int(datetime.now().timestamp())}.policy')
...
if __name__ == '__main__':
    train("dqnv1_v_negamax", "negamax")
    train("dqnv1_v_random", "random")
